...
class DlgMain(QDialog):
    def __init__(self):
        ...
        self.btnPyuic.clicked.connect(self.evt_btnPyrcc_clicked)
        self.btnPyuic.setFont(self.customfont)

        self.btnUIFile = QPushButton("...")
        self.btnUIFile.clicked.connect(self.evt_btnQrcFile_clicked)
    ...
    #####  Event handlers
    def evt_btnPyrcc_clicked(self):
        sFile, sExt = QFileDialog.getOpenFileName(self, "pyrcc5", os.getcwd()+"/usr/bin", "Executable Files (*)")
        if sFile:
            self.ledPyuic.setText(sFile)

    def evt_btnQrcFile_clicked(self):
        sFile, sExt = QFileDialog.getOpenFileName(self, "User Interface File", "/root/ui", "User Interface files (*.qrc)")
        ...
        if sFile:
            ...
            self.ledPyFile.setText("/root/ui/"+bn.split(".")[0]+".py")
